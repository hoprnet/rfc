...
**Channel graph**: The directed graph maintained by each node whose vertices are HOPR nodes (identified by their offchain public key) and whose directed edges represent viable payment-channel connections, each carrying quality observations as defined in [RFC-0010](../RFC-0010-automatic-path-discovery/0010-automatic-path-discovery.md) §4.2.
...
**Path-cost**: The multiplicative product of edge scores along a candidate path, in `(0, 1]`. A zero path-cost means the path is unusable.

**Forward path**: A path whose source node is the local node itself, used to carry messages toward a remote destination.

**Return path**: A path whose source node is a remote destination, used to carry replies back to the local node via a SURB (see [RFC-0004](../RFC-0004-hopr-packet-protocol/0004-hopr-packet-protocol.md)).

**Candidate pool**: The set of simple paths of a requested length enumerated from the channel graph, each paired with its path-cost. Candidates with a zero or negative path-cost are discarded before further processing.
...
Path-finding operates over the channel graph described in [RFC-0010](../RFC-0010-automatic-path-discovery/0010-automatic-path-discovery.md) §4.2. This section summarises the properties consumed by path-finding; the full graph construction and probing protocol are defined in RFC-0010 and are out of scope here.
...
Each measurement stream maintains an exponential moving average (EMA) of observed latency (window depth `N = 3`) and an EMA of probe success rate (window depth `N = 5`). These EMAs are updated by the probing mechanism and read by path-finding.
...
### 4.3 Path-cost function

Path-finding uses three variants of an edge-cost function, supplied as an opaque parameter to the graph traversal algorithm. Each variant enforces a set of edge admission criteria and computes a per-edge cost in `(0, 1]` for admissible edges, or a non-positive sentinel for inadmissible ones. The traversal algorithm prunes any path containing a non-positive edge cost.
...
5. **Path-cost accumulation**: the path-cost is the product of all per-edge costs along the path:

```
path_cost = ∏ edge_cost(e)   for each edge e in path
```

Because all costs are in `(0, 1]`, the path-cost decreases monotonically with path length and with the number of low-quality edges.
...
**Phase 1** enumerates simple paths of exactly `hops + 1` edges from `source` to `dest` using `EdgeValueFn::forward`. Paths with a zero or negative path-cost are discarded. Up to `max_paths` candidates are collected.
...
The path planner holds a validated candidate pool as a `WeightedCollection<ValidatedPath>` where each entry's weight equals its path-cost (§4.3). Path selection draws one path by probability proportional to weight.
...
This scheme ensures that a path with twice the path-cost of another is selected with twice the probability, while all paths with positive cost retain a non-zero selection chance. Paths with zero or negative cost MUST NOT be inserted into the collection.
...
**Path manipulation by adversarial relay nodes**: a malicious relay node may report artificially favourable probe results, inflating its edge score and increasing the probability of being selected. The primary mitigation is the probing mechanism defined in [RFC-0010](../RFC-0010-automatic-path-discovery/0010-automatic-path-discovery.md) §7, which requires that observations be derived from direct network measurements rather than self-reports. Path selection in this RFC is downstream of that mitigation.

**Deanonymisation via deterministic selection**: if the path selector were deterministic (always returning the highest-cost path), a passive adversary observing two flows with the same cost-optimal path could link them to the same sender. Weighted-random selection (§4.5) ensures that flows across the same source–destination pair will, over time, use different relay sets, reducing this linkage.

**Cache poisoning**: a compromised or faulty validator could insert a `ValidatedPath` referencing a non-existent or closed channel. The cache TTL (default 60 s) bounds the lifetime of any such entry. Background refresh re-validates cached entries on every sweep, and on-chain validation at cache-miss time provides an authoritative check.
...
**Last-hop omission of payment channel**: the last edge `(relay → destination)` is not required to have an open payment channel (§4.6). This is by design (see RFC-0010 §4.2) but means the last relay receives no payment for the final delivery hop. A relay SHOULD be aware that serving as a last-hop intermediary is uncompensated.
...
**No stake or capacity weighting**: the current implementation weights paths purely by their quality score derived from probe latency and success rate. Channel balance or stake is not a factor in path-cost. This means a low-capacity but well-probed edge may score identically to a high-capacity one, potentially causing ticket failures if a channel becomes underfunded.
...
**Yen's K-shortest-paths algorithm**: Yen's algorithm produces the K cheapest simple paths between two nodes in polynomial time per path, using a Dijkstra-based auxiliary graph. For graphs with tens of thousands of nodes and sparse channel topologies, Yen's may outperform DFS at large hop counts. It was not adopted in 4.0.x because the hop bound of 3 makes the DFS tractable, and Yen's requires maintenance of heap-based auxiliary structures that add implementation complexity.

**Bayesian edge scoring**: rather than an EMA of latency and success rate, a Bayesian model would maintain a posterior distribution over edge reliability given observed outcomes. This would provide confidence intervals and better handling of cold-start (new edges with few observations). It was not adopted because the EMA approach is simpler to implement and reason about, and the probe-based bootstrapping from RFC-0010 provides sufficient observation density in practice.

**Stake-aware path weighting**: adding channel balance or node stake to the path-cost function would reduce the probability of selecting underfunded channels, decreasing ticket failure rates. This was considered but deferred because it introduces a dependency on the settlement layer (which may be observed with latency) and requires careful normalisation to avoid suppressing otherwise good-quality low-capacity edges. See §11 for future work.
...
**Reactive path repair**: on relay failure (detected mid-session from a missing acknowledgement), the planner could trigger an immediate cache eviction and re-selection. The current design relies on the background refresh and TTL expiry, which may leave a stale path in use for up to 90 s. Reactive repair was deferred due to the complexity of correlating relay failures (which surface as transport errors in upper layers) with specific cache keys.

## 10. Unresolved questions

- What is the optimal cost-function composition for paths longer than 3 intermediate hops, should the hop bound ever be raised? The current multiplicative formula may over-penalise long paths through medium-quality edges.

- Should path-selection telemetry (e.g., a histogram of chosen path-costs over time) be exported as an observable metric? Such a metric would aid in tuning `edge_penalty` and `min_ack_rate` but also reveals routing patterns to the node operator.

## 11. Future work

- **Stake-aware path weighting**: incorporate channel balance or node stake into the path-cost function to reduce ticket failure on underfunded channels. Also listed in [RFC-0010](../RFC-0010-automatic-path-discovery/0010-automatic-path-discovery.md) §11.
