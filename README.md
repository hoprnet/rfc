...
Request for Comments (RFC) for the HOPR protocol.

Welcome to the HOPR RFC Repository — the central hub for managing, discussing,
and finalising RFCs for the HOPR protocol, its interfaces, and related
smart-contract specifications.

This README doubles as a **single-file protocol summary**: a precise condensation
of RFC-0001 through RFC-0016 that lets a reader grasp the whole stack without
opening every document. It is descriptive, not normative; the individual RFCs are
authoritative. Statuses are as of 2026-07-06.
...
and the same gap applies to the rendezvous-bridge role of RFC-0015.
...
`0x2–0xd` user-defined (RFC-0015 Onion Service Control Protocol uses `0x2`);
...
## 9. Onion Services and the Directory (RFC-0015, RFC-0016)

RFC-0015 builds **mutually anonymous onion services** on the stack above, and
RFC-0016 provides the directory they need.

- **Rendezvous architecture (RFC-0015).** A service keeps standing sessions to
  announced **introduction bridges** and publishes a signed **descriptor**; a
  client picks a fresh **rendezvous bridge** and the two meet there, the bridge
  splicing two independent HOPR legs and seeing only ciphertext behind a
  Noise-IK/X25519 end-to-end handshake. Neither endpoint learns the other's
  location (multi-hop legs, with a non-guard honest hop); constant-rate shaping
  and optional **multipath** (one session striped across several bridges) bound
  the bridge's correlation vantage.
- **Naming.** Self-certifying `base32(pk).hopr` addresses (Ed25519), optional
  ENS aliases, and delegation certificates for multi-host serving.
- **Bridges.** No bond and no bridge directory: bridges are discovered by direct
  capability negotiation, vouched for in the descriptor's rendezvous set (the
  client MAY add its own), and disciplined by local reputation; the incentive is
  a PIX-settled per-delivered-frame fee.
- **Directory (RFC-0016).** A privacy-preserving DHT storing service descriptors
  at **blinded slots** (Tor-v3 Ed25519 key blinding) fetched over anonymous
  sessions — crawl-, censorship-, and eclipse-resistant under an explicit
  honest-fraction assumption.

These build directly on the properties the stack already provides — sender
anonymity, SURB reply channels, per-hop PoR incentives, PIX conditional
destination incentives, session semantics, and on-chain announcement — and close
the gaps it left open: mutual anonymity, naming/discovery, and a rendezvous
incentive.

## Repository Structure

Each RFC lives in its own directory for modularity, easy management, and
independent versioning, with associated assets (images, diagrams) alongside.

```plaintext
/hopr-rfc
│
├── rfcs
│   ├── RFC-0001-rfc-process/             # RFC process documentation
│   │   ├── assets/                       # Related assets (images, diagrams)
│   │   └── 0001-rfc-process.md           # RFC document
│   └── RFC-XXXX-title-slug/
│       ├── assets/
│       └── XXXX-title-slug.md
├── scripts                               # Scripts checking RFC formatting/structure
├── latex                                 # LaTeX / PDF generation for the RFCs
├── ui                                    # UI for displaying the RFCs
└── templates                             # Markdown starter template for new RFCs
```

## Contributing to RFCs

The contribution process is detailed in `./rfcs/RFC-0001-rfc-process/`, and the
template to start a new one is in `./templates/rfc-template.md`.
