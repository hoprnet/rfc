...
The HOPR packet format is the fundamental building block of the HOPR protocol, enabling the construction of the HOPR mixnet. The format is designed to
...
These two parts are concatenated to form the complete HOPR packet, which has a fixed size regardless of the actual payload length (achieved through
padding) to prevent traffic analysis based on packet size.
