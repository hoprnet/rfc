# HOPR Protocol Summary (Working Reference for RFC-0015)

A single-file, precise condensation of RFC-0001 through RFC-0014 (including the
RFC-0012 PIX draft), intended as the working reference for authoring RFC-0015
(Onion Services). Statuses are as of 2026-07-05. This document is descriptive,
not normative; the referenced RFCs are authoritative.
...
and the same gap applies to any service-side role an onion service RFC
introduces.
...
`0x2–0xd` user-defined; `0xe` catch-all; `0xf … 2^61−1` session protocol
(RFC-0008). Flags carry local packet-layer ↔ upper-layer signals (SURB
...
## 8. Economic Reward System (RFC-0007, non-normative here)
...
earn normal PoR tickets from them). Relevant to RFC-0015 mainly as precedent:
staking-gated eligibility and stake-proportional traffic allocation.

## 9. Properties Available to an Onion-Service Design

What HOPR already gives us:

1. **Sender anonymity** toward relays and destination (SPHINX, 0–3 hops each
   way, per-hop mixing, fixed packet size).
2. **Recipient-initiated reply channel without deanonymisation**: pseudonyms +
   SURBs, including incentivised return paths (SURB carries PoRValues; return
   traffic is pre-paid by the SURB creator).
3. **Per-hop incentives** with on-chain settlement (channels + probabilistic
   tickets + PoR) — spam has a price floor set by ticket value and channel
   collateral.
4. **Conditional destination-side incentives** gated on verifiable reply
   handover (PIX pattern: VSS shares + stealth address + privacy pool).
5. **Session semantics** over the mixnet (reliable/unreliable, keep-alive,
   capability bits, CBOR targets — a natural place for an onion-service
   target type).
6. **On-chain node announcement** and quality-scored path selection.

What HOPR does **not** provide (gaps RFC-0015 must close):

1. Both endpoints of a session know the Exit's identity; there is no built-in
   way for *two mutually anonymous parties* to communicate — someone must be
   the addressable endpoint. Hence a rendezvous/bridge role.
2. No service descriptor / naming / discovery layer.
3. No incentive for the rendezvous-splice role (neither PoR ticket streams
   nor PIX covers "hold a session open and splice two of them").
4. SURB supply is a hard flow constraint: 0–15 SURBs per packet, distress
   signalling exists, but a high-traffic inbound service needs an explicit
   SURB replenishment strategy.
5. Session Start's `target` is Exit-interpreted (clearnet-style); an onion
   target type and its semantics are undefined.
